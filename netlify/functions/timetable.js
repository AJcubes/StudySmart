...
export async function getTimetable(email) {
    ...
    for (const event of Object.values(timetable)) {
        if (event.type === "VEVENT" && event.start > new Date()) {
            ...
            const assignmentURL = assignmentURLMatch?.[1]?.trim() ? `<h5 style="margin: 0;">View Assignment: <a href="${assignmentURLMatch[1].trim()}" target="_blank" style="color: #758e96;">${assignmentURLMatch[1].trim()}</a></h5>` : "";
